...
def export_database():
    ...
    for bug_entry in parsed_content:
        ...
        c.execute("""INSERT INTO DBMS_BUGS (DBMS, ORACLE, STATUS, DATE, TEST,
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                  (bug_entry['dbms'], bug_entry['oracle'],
                   ...
                   severity,
                   bug_entry.get('links', {}).get('email', None),
                   bug_entry.get('links', {}).get('bugtracker', None),
                   bug_entry.get('links', {}).get('fix', None)))
        ...
        for statement in bug_entry['test'].split('\n'):
            if not (statement.startswith('--') or statement == ''):
                c.execute('INSERT INTO BUG_TEST_CASES'
